...
class PatchTST_backbone(nn.Module):
    def __init__(self, c_in:int, context_window:int, target_window:int, patch_len:int, stride:int, max_seq_len:Optional[int]=1024, 
        ...
        # Head
        self.head_nf = d_model * patch_num if not channel_mixing else c_in * d_model
        self.n_vars = c_in
...
class ChannelMixingHead(nn.Module):
    def __init__(self, n_vars, patch_num, d_model, target_window, head_dropout=0):
        ...
        self.linear = nn.Linear(patch_num * d_model, n_vars * target_window)       # [patch_num * d_model] -> [n_vars * target_window]
    ...
    def forward(self, x):                                                          # x: [bs x patch_num x d_model]
        ...
        x = self.linear(x_flat)  # x: [bs, n_vars * target_window]

        x = self.dropout(x)  

        x = x.view(bs, self.n_vars, self.target_window)  # x: [bs, n_vars, target_window]
...
class TSTiEncoder(nn.Module):  #i means channel-independent
    ...
    def forward(self, x) -> Tensor:                                              # x: [bs x nvars x patch_len x patch_num]
        ...
        z = self.encoder(u)                                                      # z: [bs * nvars x patch_num x d_model]
                                                                                 ...
                                                                                 # Final output of encoder. No patch modeling
